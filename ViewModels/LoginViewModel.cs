...
public partial class LoginViewModel : BaseViewModel
{
    ...
    private async Task LoginAsync()
    {
        ...
        // NOTE (merge): the previous cross-tenant unsynced-sales confirm prompt
        // was dropped. Remote's logout model (ClearLegacyUserData) preserves
        // unsynced sales (Synced = 0) across tenant switches by contract, so
        // switching tenants no longer destroys pending sales — the warning
        // (and its GetUnsyncedSalesCountForOtherTenant dependency) is obsolete.
